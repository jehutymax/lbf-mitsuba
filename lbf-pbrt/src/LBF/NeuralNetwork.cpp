...
NeuralNetwork::NeuralNetwork(char* inputFolder, int width, int height, int featureLength)
{
	...
	// Initialize activation functions in the hidden and output layers
...
void NeuralNetwork::ApplyWeightsAndFilter(char* inputFolder, char* sceneName, float* featureData, float* varData) {
	...
	// Send secondary features to NN to get filter weights
...
void NeuralNetwork::LoadNodeData(Matrix<float>* devData, Matrix<float>* hostData, char* fileName) 
{
	...
	FILE* fpw = OpenFile("filename_OpenText.txt", "w+");
	...
	for (int i = 0; i < 2 * (numOfLayers - 1); i++) {
		...
		fwrite(&width, sizeof(int), 1, fpw);
		fwrite(&height, sizeof(int), 1, fpw);
		...
		fwrite(currentElements, sizeof(float), width * height, fpw);
